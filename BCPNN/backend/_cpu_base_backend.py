...
class DenseLayer:
    ...
    def __init__(self, in_features, hypercolumns, minicolumns, taupdt, initial_counters, dtype=np.float64):
        ...
        self.weights = (0.1 * np.random.randn(self.in_features, self.out_features)).astype(dtype)
    ...
    def compute_activation(self, inputs):
        ...
        activations = self._update_state(activations, self.weights, self.bias, inputs)
        activations = self._softmax_minicolumns(activations, self.hypercolumns, self.minicolumns)
        return activations
    ...
    def train_step(self, inputs, outputs):
        self.Ci, self.Cj, self.Cij = self._update_counters(self.Ci, self.Cj, self.Cij, inputs, outputs, self.taupdt)

    def train_finalize(self):
        self.weights = self._update_weights(self.weights, self.Ci, self.Cj, self.Cij, self.taupdt/2)
            ...
            with cm as pbar:
                ...
                for i in range(number_of_batches):
                    ...
                    for l in range(layer+1):
                        ...
                        activation = self._layers[l].compute_activation(prev_activation)
    
                    if epoch > 0 and i % (number_of_batches // (n_hypercolumns + 1)) == 0:
                    ...
                    if layer + 1 == len(self._layers):
                        self._layers[layer].train_step(prev_activation, batch_labels)
                    else:
