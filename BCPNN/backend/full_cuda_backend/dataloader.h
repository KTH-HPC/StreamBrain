...
void *dataloader_queue_worker_thread(void *thread_data);
...
class dataloader {
 public:
  dataloader(dataset_t<REAL, REAL> &dataset, unsigned int queue_workers,
...
void *dataloader_queue_worker_thread(void *thread_data) {
  ...
  CUDA_CALL(cudaMallocHost(
      (void **)&images, dataset->number_of_examples * features * sizeof(REAL)));
  CUDA_CALL(cudaMallocHost(
      (void **)&labels, dataset->number_of_examples * classes * sizeof(REAL)));
  ...
  while (loader->running_) {
    ...
    for (size_t i = 0; i < dataset->number_of_examples; ++i) {
      ...
      for (size_t k = 0; k < features; ++k) {
        images[i * features + k] = dataset->images[j * features + k];
      }
      for (size_t k = 0; k < classes; ++k) {
        labels[i * classes + k] = dataset->labels[j * classes + k];
      }
    }

    lock.lock();
    while (loader->running_.load() && loader->queue_old_.size() == 0) {
      loader->queue_cv_old_.wait(lock);
    }
    if (!loader->running_.load()) {
      break;
    }
    ...
    CUDA_CALL(cudaMemcpy(p.first, images,
                         dataset->number_of_examples * features * sizeof(REAL),
                         cudaMemcpyHostToDevice));
    CUDA_CALL(cudaMemcpy(p.second, labels,
                         dataset->number_of_examples * classes * sizeof(REAL),
                         cudaMemcpyHostToDevice));
...
template <typename REAL>
dataloader<REAL>::dataloader(dataset_t<REAL, REAL> &dataset,
                             unsigned int queue_workers,
                             unsigned int queue_size) {
  ...
  for (size_t i = 0; i < queue_size; ++i) {
    ...
    CUDA_CALL(cudaMalloc((void **)&p.first,
                         dataset.number_of_examples * n_inputs * sizeof(REAL)));
    CUDA_CALL(cudaMalloc((void **)&p.second, dataset.number_of_examples *
  ...
  for (size_t i = 0; i < queue_workers; ++i) {
    ...
    pthread_create(&thread_id, &attr,
                   &internal::dataloader_queue_worker_thread<REAL>,
                   (void *)this);
  }
}

template <typename REAL>
dataset_t<REAL, REAL> &dataloader<REAL>::get_dataset() {
  return *dataset_;
}

template <typename REAL>
std::pair<REAL *, REAL *> dataloader<REAL>::queue_get_fresh() {
  std::unique_lock<std::mutex> lock(queue_mutex_);
  while (running_ && queue_fresh_.size() == 0) {
    queue_cv_fresh_.wait(lock);
  }
  if (!running_ && queue_fresh_.size() == 0) {
    return std::pair<REAL *, REAL *>(NULL, NULL);
  }
...
template <typename REAL>
void dataloader<REAL>::queue_recycle(std::pair<REAL *, REAL *> p) {
  std::lock_guard<std::mutex> lock(queue_mutex_);
  queue_old_.push(p);
