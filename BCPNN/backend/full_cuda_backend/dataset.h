...
dataset_t<REAL, REAL>
convert_dataset(dataset<uint8_t, uint8_t> dataset)
{
  dataset_t<REAL, REAL> d;

  d.images = (REAL *)malloc(sizeof(REAL) * dataset.rows * dataset.cols * dataset.number_of_examples);
  d.labels = (REAL *)malloc(sizeof(REAL) * dataset.number_of_classes * dataset.number_of_examples);
  ...
  for (size_t example = 0; example < d.number_of_examples; ++example) {
    for (size_t c = 0; c < d.number_of_classes; ++c) { label_2[c] = 0; }
    label_2[*label_1] = 1;
...
template<typename D, typename L>
void
print_dataset(dataset_t<D, L> const& dataset)
{
  std::cout << "Number of examples: " << dataset.number_of_examples << std::endl;
  ...
  if (dataset.one_hot_label) {
    ...
    std::cout << "Label: " << ((unsigned int)dataset.labels[example]) << std::endl;
  }
}
