...
void naive_add_bias(REAL *matrix, size_t n, size_t m, REAL *bias) {
...
template <typename REAL>
void naive_softmax(REAL *matrix, size_t n, size_t m) {
  for (size_t i = 0; i < n; ++i) {
...
template <typename REAL>
void naive_update_counters(REAL *Ci, REAL *Cj, REAL *Cij, REAL *inputs,
  ...
  for (size_t i = 0; i < n; ++i) {
    for (size_t j = 0; j < m; ++j) {
      Cij[i * m + j] = fmaf(1 - taupdt, Cij[i * m + j],
                            tij[i * m + j] / batch_size * taupdt);
    }
  }
}

template <typename REAL>
void naive_update_weights(REAL *weights, REAL *Ci, REAL *Cj, REAL *Cij,
...
template <typename REAL>
void naive_update_bias(REAL *bias, REAL *Cj, REAL cthr, size_t m) {
  for (size_t j = 0; j < m; ++j) {
    if (Cj[j] < cthr) {
      bias[j] = logf(2 * cthr);
    } else {
...
template <typename REAL>
void naive_update_bias_regularized(REAL *bias, REAL *kbi, REAL *Cj, REAL cthr,
...
template <typename REAL>
void naive_update_mask(uint8_t *wmask, REAL *weights, REAL *Ci, REAL *Cj,
  ...
  for (size_t iter = 0; iter < iterations; ++iter) {
    ...
    for (size_t i = 0; i < n; ++i) {
      ...
      if (wmask[i * hypercolumns + h] == 0 && (imax0 == n || score >= vmax0)) {
        imax0 = i;
        vmax0 = score;
      } else if (wmask[i * hypercolumns + h] == 1 &&
                 (imin1 == n || score <= vmin1)) {
        imin1 = i;
        vmin1 = score;
      }
...
template <typename REAL>
void naive_apply_mask(REAL *weight, uint8_t *wmask, size_t n, size_t m,
                      size_t hypercolumns, size_t minicolumns) {
