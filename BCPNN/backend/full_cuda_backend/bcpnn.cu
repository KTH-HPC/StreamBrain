...
void train_layer_1(dataloader<REAL> & loader, REAL * W1, REAL * B1, REAL taupdt, REAL khalf, REAL pmin, REAL taubdt, size_t epochs);
void train_layer_2(dataloader<REAL> & loader, REAL * W1, REAL * B1, REAL * W2, REAL * B2, REAL taupdt, size_t epochs);
double evaluate_network(dataset_t<REAL, REAL> & test_dataset, REAL * W1, REAL * B1, REAL * W2, REAL * B2);
...
main(int argc, char * argv[])
{
  ...
  CURAND_CALL(TcurandGenerateNormal<REAL>(gen, W1, n_inputs * n_hidden, 0, 0.1));
  CURAND_CALL(TcurandGenerateUniform<REAL>(gen, B1, n_hidden));
  CURAND_CALL(TcurandGenerateNormal<REAL>(gen, W2, n_hidden * n_outputs, 0, 0.1));
  ...
  train_layer_1(loader, W1, B1, taupdt, l1_khalf, l1_pmin, l1_taubdt, l1_epochs);
  ...
  std::cout << "Training duration: " << std::chrono::duration_cast<std::chrono::milliseconds>(training_end - training_start).count() / 1000.0 << std::endl;
  std::cout << "Testing duration: " << std::chrono::duration_cast<std::chrono::milliseconds>(testing_end - testing_start).count() / 1000.0 << std::endl;
...
void
training_step_layer_1(REAL * W1, REAL * B1, size_t batch_size, REAL * batch_images, REAL * batch_labels, REAL * activation_1, REAL * Ci, REAL * Cj, REAL * Cij, uint8_t * wmask, REAL * kbi, REAL taupdt, REAL khalf, REAL pmin, REAL taubdt, size_t hypercolumn);

void
training_step_layer_2(REAL * W1, REAL * B1, REAL * W2, REAL * B2, size_t batch_size, REAL * batch_images, REAL * batch_labels, REAL * activation_1, REAL * Ci, REAL * Cj, REAL * Cij, REAL taupdt);

void
evaluate_batch(int * correct_count, REAL * W1, REAL * B1, REAL * W2, REAL * B2, size_t batch_size, REAL * batch_images, REAL * batch_labels, REAL * activation_1, REAL * activation_2);

void
train_layer_1(dataloader<REAL> & loader, REAL * W1, REAL * B1, REAL taupdt, REAL khalf, REAL pmin, REAL taubdt, size_t epochs)
{
  REAL * activation_1;
  REAL * Ci;
  REAL * Cj;
  ...
  CUDA_CALL(cudaMalloc((void **)&activation_1, batch_size * n_hidden * sizeof(REAL)));
  ...
  CUDA_CALL(cudaMalloc((void **)&wmask, n_inputs * n_hypercolumns * sizeof(uint8_t)));
  ...
  cuda_initialize_array<REAL>(Cij, 1/((REAL)n_minicolumns), n_inputs * n_hidden);
  cuda_initialize_array<REAL>(kbi, 1, n_hidden);

  initialize_wmask(wmask, n_inputs, n_hypercolumns);

  std::vector<size_t> hc_shuf; for (size_t i = 0; i < n_hypercolumns; ++i) { hc_shuf.push_back(i); }
  for (size_t epoch = 0; epoch < epochs; ++epoch) {
    ...
    size_t n_steps = (loader.get_dataset().number_of_examples + batch_size - 1) / batch_size;
    for (size_t step = 0; step < n_steps; ++step) {
      REAL * batch_images = p.first + (pos * n_inputs);
      REAL * batch_labels = p.second + (pos * n_outputs);
      size_t batch_size_step = min(batch_size, loader.get_dataset().number_of_examples - pos);
      ...
      training_step_layer_1(W1, B1, batch_size_step, batch_images, batch_labels, activation_1, Ci, Cj, Cij, wmask, kbi, taupdt, khalf, pmin, taubdt, (h < n_hypercolumns ? hc_shuf[h] : n_hypercolumns));
...
void
train_layer_2(dataloader<REAL> & loader, REAL * W1, REAL * B1, REAL * W2, REAL * B2, REAL taupdt, size_t epochs)
{
  REAL * activation_1;
  REAL * Ci;
  REAL * Cj;
  ...
  cuda_initialize_array<REAL>(Cij, 1/((REAL)n_hidden*n_outputs), n_hidden * n_outputs);
  ...
  for (size_t epoch = 0; epoch < epochs; ++epoch) {
    ...
    for (size_t step = 0; step < n_steps; ++step) {
      ...
      training_step_layer_2(W1, B1, W2, B2, batch_size_step, batch_images, batch_labels, activation_1, Ci, Cj, Cij, taupdt);
...
double
evaluate_network(dataset_t<REAL, REAL> & test_dataset, REAL * W1, REAL * B1, REAL * W2, REAL * B2)
{
  int * correct;
  REAL * activation_1;
  REAL * activation_2;
  ...
  CUDA_CALL(cudaMalloc((void **)&activation_1, batch_size * n_hidden * sizeof(REAL)));
  CUDA_CALL(cudaMalloc((void **)&activation_2, batch_size * n_outputs * sizeof(REAL)));
  ...
  CUDA_CALL(cudaMalloc((void **)&test_images, test_dataset.number_of_examples * n_inputs * sizeof(REAL)));
  CUDA_CALL(cudaMalloc((void **)&test_labels, test_dataset.number_of_examples * n_outputs * sizeof(REAL)));

  CUDA_CALL(cudaMemcpy(test_images, test_dataset.images, test_dataset.number_of_examples * n_inputs * sizeof(REAL), cudaMemcpyHostToDevice));
  CUDA_CALL(cudaMemcpy(test_labels, test_dataset.labels, test_dataset.number_of_examples * n_outputs * sizeof(REAL), cudaMemcpyHostToDevice));

  size_t pos = 0;

  for (size_t step = 0; step < (test_dataset.number_of_examples + batch_size - 1); ++step) {
    REAL * batch_images = test_images + (pos * n_inputs);
    REAL * batch_labels = test_labels + (pos * n_outputs);
    size_t batch_size_step = min(batch_size, test_dataset.number_of_examples - pos);

    evaluate_batch(correct, W1, B1, W2, B2, batch_size_step, batch_images, batch_labels, activation_1, activation_2);
  ...
  /*
  float * h_activation_2 = (float *)malloc(batch_size * n_outputs * sizeof(float));
  CUDA_CALL(cudaMemcpy(h_activation_2, activation_2, batch_size * n_outputs * sizeof(float), cudaMemcpyDeviceToHost));

  for (size_t example = 0; example < 5; ++example) {
    for (size_t i = 0; i < 10; ++i) { std::cout << i << ": " << h_activation_2[example*10 + i] << std::endl; }
    std::cout << std::endl;
    for (size_t i = 0; i < 10; ++i) { std::cout << i << ": " << test_dataset.labels[(10000 - 100 + example)*10 + i] << std::endl; }
    std::cout << std::endl;
    std::cout << std::endl;
  }
  */
...
training_step_layer_1(REAL * W1, REAL * B1, size_t batch_size, REAL * batch_images, REAL * batch_labels, REAL * activation_1, REAL * Ci, REAL * Cj, REAL * Cij, uint8_t * wmask, REAL * kbi, REAL taupdt, REAL khalf, REAL pmin, REAL taubdt, size_t hypercolumn)
{
  ...
  CUBLAS_CALL(cublasgemm<REAL>(handle, CUBLAS_OP_N, CUBLAS_OP_N, n_hidden, batch_size, n_inputs, &v_one, W1, n_hidden, batch_images, n_inputs, &v_zero, activation_1, n_hidden));

  cuda_add_bias(activation_1, batch_size, n_hidden, B1);

  cuda_softmax(activation_1, batch_size * n_hypercolumns, n_minicolumns);

  cuda_update_counters(Ci, Cj, Cij, batch_images, activation_1, batch_size, n_inputs, n_hidden, taupdt);
  cuda_update_weights(W1, Ci, Cj, Cij, taupdt/2, n_inputs, n_hidden);
  cuda_update_bias_regularized(B1, kbi, Cj, taupdt/2, khalf, pmin, taubdt, n_hidden);
  if (hypercolumn < n_hypercolumns) {
    cuda_update_mask(wmask, W1, Ci, Cj, Cij, taupdt/2, n_inputs, n_hidden, hypercolumn, n_hypercolumns, n_minicolumns, 16);
  }
...
training_step_layer_2(REAL * W1, REAL * B1, REAL * W2, REAL * B2, size_t batch_size, REAL * batch_images, REAL * batch_labels, REAL * activation_1, REAL * Ci, REAL * Cj, REAL * Cij, REAL taupdt)
{
  ...
  cuda_update_counters(Ci, Cj, Cij, activation_1, batch_labels, batch_size, n_hidden, n_outputs, taupdt);
...
evaluate_batch(int * correct_count, REAL * W1, REAL * B1, REAL * W2, REAL * B2, size_t batch_size, REAL * batch_images, REAL * batch_labels, REAL * activation_1, REAL * activation_2)
{
  ...
  for (size_t i = 0; i < n_cycles_testing; ++i) {
    ...
    CUBLAS_CALL(cublasgemm<REAL>(handle, CUBLAS_OP_N, CUBLAS_OP_N, n_outputs, batch_size, n_hidden, &v_one, W2, n_outputs, activation_1, n_hidden, &v_zero, activation_2, n_outputs));
...
#ifdef ACCURACY_CPU_DOUBLE_CHECK
  cpu_correct_predictions(&cpu_correct, activation_2, batch_labels, batch_size, n_outputs);
#endif
  cuda_correct_predictions(correct_count, activation_2, batch_labels, batch_size, n_outputs);
}
