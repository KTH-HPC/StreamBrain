...
{
    bcpnn::helpers::blas::matmul<REAL>(activation, inputs, weights, inputs_rows, inputs_cols, weights_cols); 
...
template <typename REAL>
REAL vectorized_updateWeights(REAL C, REAL *Ci, REAL *Cj, REAL *Cij,
                              REAL *a_i, REAL *a_o,
...
template <typename REAL>
void updateState(py::array_t<REAL> py_activation,
                 ...
                 py::array_t<REAL> py_bias)
{
    ...
    bcpnn::kernels::cpu::matmul_add_bias(activation, inputs, weights, bias, inputs_buffer.shape[0], inputs_buffer.shape[1], weights_buffer.shape[0], weights_buffer.shape[1]);
}
                   ...
                   const REAL cthr)
{
    ...
    return bcpnn::kernels::cpu::vectorized_updateWeights(C, Ci, Cj, Cij, a_i, a_o, taupdt, a_i_buffer.shape[0], Ci_buffer.shape[0], Cj_buffer.shape[0], weights, cthr);
}
                           ...
                           REAL taubdt)
{
    ...
    bcpnn::kernels::cpu::update_bias_regularized<REAL>(cpu_bias, cpu_kbi, cpu_Cj, cthr, khalf, pmin, taubdt, m);
}
                ...
                size_t iterations)
{
    ...
    bcpnn::kernels::cpu::update_mask<REAL>(cpu_wmask, cpu_weights, cpu_Ci, cpu_Cj, cpu_Cij, cthr, n, m, h, hypercolumns, minicolumns, iterations);
}
               ...
               size_t minicolumns)
{
    ...
    bcpnn::kernels::cpu::apply_mask<REAL>(cpu_weights, cpu_wmask, n, m, hypercolumns, minicolumns);
}
