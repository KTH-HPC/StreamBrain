...
void compute_offsets(size_t size, int world_rank, int world_size, int *displs,
                     int *rcounts) {
  /* compute stride */
...
REAL updateWeights(REAL C, REAL *Ci, REAL *Cj, REAL *Cij, REAL *a_i, REAL *a_o,
...
#pragma omp single
    {
      MPI_Allreduce(MPI_IN_PLACE, Ci_tmp_array, in_features, datatype, MPI_SUM,
                    MPI_COMM_WORLD);
      MPI_Allreduce(MPI_IN_PLACE, Cj_tmp_array, out_features, datatype, MPI_SUM,
                    MPI_COMM_WORLD);
      MPI_Allreduce(MPI_IN_PLACE, tmp_array, in_features * out_features,
...
#pragma unroll
    ...
    // Ci[i] = one_minus_taupdt * Ci[i] + (tmp_array[i] / (REAL)(batch_size *
    // world_size) * taupdt);
...
#pragma unroll
    ...
    // Cj[i] = one_minus_taupdt * Cj[i] + (tmp_array[i] / (REAL)(batch_size *
    // world_size) * taupdt);
            ...
            weights[i * out_features + vl] =
                (Cj[vl] < cthr) ? 0.0
                                : std::log(std::max(Cij[i * out_features + vl],
                                                    cthr_pow_four) /
                                           (Ci[i] * Cj[vl]));
        // weights[i * out_features + vl] = (Cj[vl] < cthr) ? 0.0 : std::log((C
        // * C_p[vl-j]) / (Ci[i] * Cj[vl]));
      }
    }
  }  // end omp parallel
...
void update_bias(REAL *bias, REAL *Cj, REAL cthr, const long out_features) {
  ...
  bcpnn::helpers::offsets::compute_offsets(out_features, world_rank, world_size,
                                           displs, rcounts);
...
#pragma omp for
  ...
  MPI_Allgatherv(MPI_IN_PLACE, stride, datatype, bias, rcounts, displs,
...
void update_bias_regularized(REAL *bias, REAL *kbi, REAL *Cj, REAL cthr,
  ...
  bcpnn::helpers::offsets::compute_offsets(m, world_rank, world_size, displs,
                                           rcounts);
...
#pragma omp for
  ...
  MPI_Allgatherv(MPI_IN_PLACE, stride, datatype, kbi, rcounts, displs, datatype,
                 MPI_COMM_WORLD);
  MPI_Allgatherv(MPI_IN_PLACE, stride, datatype, bias, rcounts, displs,
                 datatype, MPI_COMM_WORLD);
}
...
void update_mask(uint8_t *wmask, REAL *weights, REAL *Ci, REAL *Cj, REAL *Cij,
  ...
  bcpnn::helpers::offsets::compute_offsets(n, world_rank, world_size, displs,
                                           rcounts);
...
#pragma omp for
  ...
  /* sync after update */
  MPI_Allgatherv(MPI_IN_PLACE, rcounts[world_rank], datatype, tmp_score,
                 rcounts, displs, datatype, MPI_COMM_WORLD);
...
#pragma omp for
  ...
  MPI_Allgatherv(MPI_IN_PLACE, rcounts[world_rank], datatype, weight, rcounts,
                 displs, datatype, MPI_COMM_WORLD);
}
...
REAL updateWeights(REAL C, py::array_t<REAL> py_Ci, py::array_t<REAL> py_Cj,
  ...
  return bcpnn::kernels::mpi::updateWeights(
...
void updateBiasRegularized(py::array_t<REAL> py_bias, py::array_t<REAL> py_kbi,
  ...
  bcpnn::kernels::mpi::update_bias_regularized<REAL>(
...
void updateMask(py::array_t<uint8_t> py_wmask, py::array_t<REAL> py_weights,
  ...
  bcpnn::kernels::mpi::update_mask<REAL>(cpu_wmask, cpu_weights, cpu_Ci, cpu_Cj,
...
void applyMask(py::array_t<REAL> py_weights, py::array_t<uint8_t> py_wmask,
  ...
  bcpnn::kernels::mpi::apply_mask<REAL>(cpu_weights, cpu_wmask, n, m,
