...
cublasStatus_t cublasgemm(cublasHandle_t handle, cublasOperation_t transa,
                          ...
                          const REAL *B, int ldb, const REAL *beta, REAL *C,
                          int ldc) {}
...
template <typename REAL>
void cuda_update_state(REAL *state, REAL *inputs, REAL *weights, REAL *bias,
                       size_t batch_size, size_t n, size_t m) {
  REAL v_one = 1;
  REAL v_zero = 0;

  CUBLAS_CALL(::bcpnn::helpers::cuda::cublasgemm<REAL>(
      global_cublas_handle, CUBLAS_OP_N, CUBLAS_OP_N, m, batch_size, n, &v_one,
      weights, m, inputs, n, &v_zero, state, m));
...
__global__ void kernel_update_wmask(REAL *wmask_score_nominator,
  ...
  for (size_t iter = 0; cntnue && iter < iterations; ++iter) {
    ...
    for (size_t d = 16; d > 0; d /= 2) {
      ...
      if (imax0 == n ||
          (iup != n && (vup > vmax0 || (vup == vmax0 && iup > imax0)))) {
        imax0 = iup;
        vmax0 = vup;
      }
      ...
      if (imin1 == n ||
          (iup != n && (vup < vmin1 || (vup == vmin1 && iup > imin1)))) {
        imin1 = iup;
        vmin1 = vup;
      }
    }
...
template <typename REAL>
void update_state(py::array_t<REAL> py_state, py::array_t<REAL> py_weight,
                  py::array_t<REAL> py_bias, py::array_t<REAL> py_inputs) {
  ...
  CUDA_CALL(cudaMemcpy(gpu_state, cpu_state, batch_size * m * sizeof(REAL),
                       cudaMemcpyHostToDevice));
  CUDA_CALL(cudaMemcpy(gpu_weight, cpu_weight, n * m * sizeof(REAL),
                       cudaMemcpyHostToDevice));
  CUDA_CALL(
      cudaMemcpy(gpu_bias, cpu_bias, m * sizeof(REAL), cudaMemcpyHostToDevice));
  CUDA_CALL(cudaMemcpy(gpu_inputs, cpu_inputs, batch_size * n * sizeof(REAL),
                       cudaMemcpyHostToDevice));

  bcpnn::kernels::cuda::cuda_update_state(gpu_state, gpu_inputs, gpu_weight,
                                          gpu_bias, batch_size, n, m);

  CUDA_CALL(cudaMemcpy(cpu_state, gpu_state, batch_size * m * sizeof(REAL),
...
template <typename REAL>
void add_bias(py::array_t<REAL> py_matrix, py::array_t<REAL> py_bias) {
  ...
  CUDA_CALL(cudaMemcpy(gpu_matrix, cpu_matrix, n * m * sizeof(REAL),
                       cudaMemcpyHostToDevice));
  CUDA_CALL(
      cudaMemcpy(gpu_bias, cpu_bias, m * sizeof(REAL), cudaMemcpyHostToDevice));
  ...
  CUDA_CALL(cudaMemcpy(cpu_matrix, gpu_matrix, n * m * sizeof(REAL),
                       cudaMemcpyDeviceToHost));
  CUDA_CALL(
      cudaMemcpy(cpu_bias, gpu_bias, m * sizeof(REAL), cudaMemcpyDeviceToHost));
...
template <typename REAL>
void softmax_minicolumns(py::array_t<REAL> py_matrix, size_t hypercolumns,
                         size_t minicolumns) {
  py::buffer_info matrix_buffer = py_matrix.request();
...
template <typename REAL>
void update_counters(py::array_t<REAL> py_Ci, py::array_t<REAL> py_Cj,
                     py::array_t<REAL> py_Cij, py::array_t<REAL> py_inputs,
                     py::array_t<REAL> py_outputs, REAL taupdt) {
  ...
  CUDA_CALL(
      cudaMemcpy(gpu_Ci, cpu_Ci, n * sizeof(REAL), cudaMemcpyHostToDevice));
  CUDA_CALL(
      cudaMemcpy(gpu_Cj, cpu_Cj, m * sizeof(REAL), cudaMemcpyHostToDevice));
  CUDA_CALL(cudaMemcpy(gpu_Cij, cpu_Cij, n * m * sizeof(REAL),
                       cudaMemcpyHostToDevice));
  CUDA_CALL(cudaMemcpy(gpu_inputs, cpu_inputs, batch_size * n * sizeof(REAL),
                       cudaMemcpyHostToDevice));
  CUDA_CALL(cudaMemcpy(gpu_outputs, cpu_outputs, batch_size * m * sizeof(REAL),
                       cudaMemcpyHostToDevice));

  bcpnn::kernels::cuda::cuda_update_counters(gpu_Ci, gpu_Cj, gpu_Cij,
                                             gpu_inputs, gpu_outputs,
                                             batch_size, n, m, taupdt);

  CUDA_CALL(
      cudaMemcpy(cpu_Ci, gpu_Ci, n * sizeof(REAL), cudaMemcpyDeviceToHost));
  CUDA_CALL(
      cudaMemcpy(cpu_Cj, gpu_Cj, m * sizeof(REAL), cudaMemcpyDeviceToHost));
  CUDA_CALL(cudaMemcpy(cpu_Cij, gpu_Cij, n * m * sizeof(REAL),
                       cudaMemcpyDeviceToHost));
...
template <typename REAL>
void update_weights(py::array_t<REAL> py_weights, py::array_t<REAL> py_Ci,
                    py::array_t<REAL> py_Cj, py::array_t<REAL> py_Cij,
                    REAL cthr) {
  ...
  CUDA_CALL(cudaMemcpy(gpu_weights, cpu_weights, n * m * sizeof(REAL),
                       cudaMemcpyHostToDevice));
  CUDA_CALL(
      cudaMemcpy(gpu_Ci, cpu_Ci, n * sizeof(REAL), cudaMemcpyHostToDevice));
  CUDA_CALL(
  ...
  bcpnn::kernels::cuda::cuda_update_weights(gpu_weights, gpu_Ci, gpu_Cj,
                                            gpu_Cij, cthr, n, m);

  CUDA_CALL(cudaMemcpy(cpu_weights, gpu_weights, n * m * sizeof(REAL),
                       cudaMemcpyDeviceToHost));
...
template <typename REAL>
void update_bias(py::array_t<REAL> py_bias, py::array_t<REAL> py_Cj,
                 REAL cthr) {
  py::buffer_info bias_buffer = py_bias.request();
  py::buffer_info Cj_buffer = py_Cj.request();
...
template <typename REAL>
void update_bias_regularized(py::array_t<REAL> py_bias,
                             py::array_t<REAL> py_kbi, py::array_t<REAL> py_Cj,
                             REAL cthr, REAL khalf, REAL pmin, REAL taubdt) {
  py::buffer_info bias_buffer = py_bias.request();
  ...
  CUDA_CALL(
      cudaMemcpy(gpu_bias, cpu_bias, m * sizeof(REAL), cudaMemcpyHostToDevice));
  CUDA_CALL(
      cudaMemcpy(gpu_kbi, cpu_kbi, m * sizeof(REAL), cudaMemcpyHostToDevice));
  CUDA_CALL(
      cudaMemcpy(gpu_Cj, cpu_Cj, m * sizeof(REAL), cudaMemcpyHostToDevice));

  bcpnn::kernels::cuda::cuda_update_bias_regularized<REAL>(
      gpu_bias, gpu_kbi, gpu_Cj, cthr, khalf, pmin, taubdt, m);

  CUDA_CALL(
      cudaMemcpy(cpu_bias, gpu_bias, m * sizeof(REAL), cudaMemcpyDeviceToHost));
  CUDA_CALL(
      cudaMemcpy(cpu_kbi, gpu_kbi, m * sizeof(REAL), cudaMemcpyDeviceToHost));
...
template <typename REAL>
void update_mask(py::array_t<uint8_t> py_wmask, py::array_t<REAL> py_weights,
  ...
  CUDA_CALL(cudaMemcpy(gpu_wmask, cpu_wmask, n * hypercolumns,
                       cudaMemcpyHostToDevice));
  CUDA_CALL(cudaMemcpy(gpu_weights, cpu_weights, n * m * sizeof(REAL),
                       cudaMemcpyHostToDevice));
  CUDA_CALL(
  ...
  bcpnn::kernels::cuda::cuda_update_mask<REAL>(
      gpu_wmask, gpu_weights, gpu_Ci, gpu_Cj, gpu_Cij, cthr, n, m, h,
      hypercolumns, minicolumns, iterations);

  CUDA_CALL(cudaMemcpy(cpu_wmask, gpu_wmask, n * hypercolumns,
                       cudaMemcpyDeviceToHost));
...
template <typename REAL>
void apply_mask(py::array_t<REAL> py_weights, py::array_t<uint8_t> py_wmask,
                size_t hypercolumns, size_t minicolumns) {
  py::buffer_info weights_buffer = py_weights.request();
  py::buffer_info wmask_buffer = py_wmask.request();
  ...
  bcpnn::kernels::cuda::cuda_apply_mask<REAL>(gpu_weights, gpu_wmask, n, m,
